...
export class LocalStorageService {
   ...
   * @param session true to use session instead of local storage.
   */
  public store(key: string, value: any, session: boolean = false): void {
    if (session) {
   ...
   * @param session true to use session instead of local storage.
   */
  public remove(key: string, session: boolean = false): void {
    if (session) {
   ...
   * @param prefix key prefix.
   */
  public clear(prefix: string, session = false): void {
    if (session) {
      for (let i = 0, len = sessionStorage.length; i < len; i++) {
        const key = sessionStorage.key(i);
        if (key.startsWith(prefix)) {
